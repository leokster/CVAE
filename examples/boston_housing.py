...
if __name__ == "__main__":
   ...
   res = []
   for i in range(20):
       test_index = read_index( "/Users/tim/OneDrive - Swissgrid AG/mc-dropout/DropoutUncertaintyExps/UCI_Datasets/bostonHousing/data/index_test_{}.txt".format(i))
       train_index = read_index("/Users/tim/OneDrive - Swissgrid AG/mc-dropout/DropoutUncertaintyExps/UCI_Datasets/bostonHousing/data/index_train_{}.txt".format(i))
       ...
       vae = VAE(encoder=encoder, decoder=decoder, prior=prior, reconstruction_weight=1)
       vae.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001))
       #vae.compile(optimizer=tf.keras.optimizers.RMSprop(learning_rate=0.0001))

       vae.fit(x_train, y_train, epochs=200, callbacks = [BetaScaling()])
